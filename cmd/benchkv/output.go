...
func writeCSV(path string, rows []epochAggregate) error {
	...
	header := []string{
		"host", "host_label", "server_version", "model", "model_family", "model_arch", "model_size_label", "quant", "kv_mode_requested", "kv_mode_requested_k", "kv_mode_requested_v", "kv_mode_resolved", "kv_mode_resolved_k", "kv_mode_resolved_v", "requested_cache_type_k", "requested_cache_type_v", "effective_cache_type_k", "effective_cache_type_v", "requested_mode", "effective_mode", "kv_algo_resolved", "kv_algo_resolved_k", "kv_algo_resolved_v", "kv_path", "kv_path_k", "kv_path_v", "path_kind", "kv_symmetric", "kv_asymmetric", "symmetric_requested", "symmetric_effective", "fallback_applied", "k_only_fallback", "fallback_reason", "turboquant_path_kind", "native_turboquant_active", "reference_turboquant_active", "flash_attention_requested", "flash_attention_effective", "fa_enabled", "fa_required_for_v_turbo", "v_turbo_supported", "detected_head_dim", "architecture_class", "support_tier", "hybrid_kv_architecture", "tq_block_size", "v_reconstruction_compute_dtype", "segmented_head_active", "segmented_head_plan", "attention_surface_policy_requested", "attention_surface_policy_effective", "attention_surface_policy", "attention_surface_behavior", "attention_surface_override_applied", "attention_surface_override_reason", "attention_surface_classes", "qjl_k_requested", "qjl_k_effective", "qjl_v_requested", "qjl_v_effective", "qjl_k_enabled", "qjl_v_enabled", "residual_tail_tokens", "experimental_weight_quantization", "experimental_weight_quant_policy", "experimental_weight_quant_source", "experimental_weight_quant_active", "gpu_stats_source", "host_stats_source", "validation_kind", "validation_status", "validation_expected", "validation_observed", "validation_error", "fit_status", "corruption_status", "corruption_class", "correctness_status", "kl_divergence_vs_baseline", "niah_depth", "niah_pass", "empty_output", "truncation_detected", "residency_kind", "notes",
		...
		"gpu_metrics_available", "host_ram_used_bytes", "peak_host_ram_bytes", "host_metrics_available", "telemetry_path_jsonl", "peak_gpu_vram_bytes_total", "peak_gpu_vram_bytes_by_gpu", "avg_prompt_tps", "avg_decode_tps", "max_prompt_tps", "max_decode_tps", "eta_confidence", "stage_durations", "progress_samples", "live_status_enabled", "telemetry_sampling_interval_sec",
	...
	for _, row := range rows {
		record := []string{
			...
			row.VReconstructionComputeDType,
			fmt.Sprintf("%t", row.SegmentedHeadActive),
			row.SegmentedHeadPlan,
			row.AttentionSurfacePolicyRequested,
			row.AttentionSurfacePolicyEffective,
			row.AttentionSurfacePolicy,
			row.AttentionSurfaceBehavior,
			fmt.Sprintf("%t", row.AttentionSurfaceOverrideApplied),
			row.AttentionSurfaceOverrideReason,
			row.AttentionSurfaceClasses,
			...
			fmt.Sprintf("%t", row.QJLKEnabled),
			fmt.Sprintf("%t", row.QJLVEnabled),
			fmt.Sprintf("%d", row.ResidualTailTokens),
			row.ExperimentalWeightQuantization,
			row.ExperimentalWeightQuantPolicy,
			row.ExperimentalWeightQuantSource,
			fmt.Sprintf("%t", row.ExperimentalWeightQuantActive),
			row.GPUStatsSource,
			row.HostStatsSource,
			row.ValidationKind,
			...
			formatFloat64CSV(row.KLDivergenceVsBaseline),
			fmt.Sprintf("%d", row.NIAHDepth),
			fmt.Sprintf("%t", row.NIAHPass),
			fmt.Sprintf("%t", row.EmptyOutput),
			fmt.Sprintf("%t", row.TruncationDetected),
			row.ResidencyKind,
			row.Notes,
			...
			formatInt64CSV(row.PeakGPUVRAMBytesTotal),
			row.PeakGPUVRAMBytesByGPU,
			formatFloat64CSV(row.AvgPromptTPS),
			formatFloat64CSV(row.AvgDecodeTPS),
			formatFloat64CSV(row.MaxPromptTPS),
			formatFloat64CSV(row.MaxDecodeTPS),
			row.ETAConfidence,
			row.StageDurations,
			fmt.Sprintf("%d", row.ProgressSamples),
			fmt.Sprintf("%t", row.LiveStatusEnabled),
			fmt.Sprintf("%d", row.TelemetrySamplingIntervalSec),
